...
function correct(x, w) {
  weightCoeffs[w][0] =
    weightCoeffs[w][0] + speedTraining * (objects[x][0] - weightCoeffs[w][0]);
  weightCoeffs[w][1] =
    weightCoeffs[w][1] + speedTraining * (objects[x][1] - weightCoeffs[w][1]);
}
...
for (let i = 0; i < objects.length; i++) {
  ...
  for (let j = 0; j < 4; j++) {
    s[j] = Math.sqrt(
      Math.pow(weightCoeffs[j][0] - objects[i][0], 2) +
        Math.pow(weightCoeffs[j][1] - objects[i][1], 2)
    );
  }
